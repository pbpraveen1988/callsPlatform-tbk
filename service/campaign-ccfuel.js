...
const { PUBLIC_FOLDER_NAME, ASSET_FOLDER_PATH, VMDROP_URL, CCFUEL_API_URL, API_KEY, CCFUEL_API_KEY, LOCAL_URL, PROD_URL, CALLBACK_PATH } = require('../global/constants');
  ...
  campaigns.forEach(async campaign => {
    ...
    if (campaign.campaignRepeatCount <= 1) {
      ...
      fs.createReadStream(PUBLIC_FOLDER_NAME + ASSET_FOLDER_PATH + campaign.csvFileName)
        ...
        .on('end', () => {
          ...
          numbers.forEach(number => {
            ...
            makeVMDropRequest(params, campaign._id, number, campaign.ccfuelAPI);
          });
      ...
      failedHistories.forEach(history => {
        ...
        makeVMDropRequestByHistory(params, campaign._id, history.number, history._id, campaign.ccfuelAPI);
      });
...
const makeVMDropRequest = async (params, campaignId, number, ccfuelAPI) => {
  console.log('Gobil: makeVMDropRequest-ccfuelAPI', ccfuelAPI);
  axios.post((ccfuelAPI ? CCFUEL_API_URL : VMDROP_URL) + "/?apikey=" + (ccfuelAPI ? CCFUEL_API_KEY : API_KEY), params)
    .then(response => {
      console.log('Gobil: makeVMDropRequest response=>', response.data, ccfuelAPI);
      const history = {
...
const makeVMDropRequestByHistory = async (params, campaignId, number, historyId, ccfuelAPI) => {
  axios.post((ccfuelAPI ? CCFUEL_API_URL : VMDROP_URL) + "/?apikey=" + (ccfuelAPI ? CCFUEL_API_KEY : API_KEY), params)
    .then(response => {
